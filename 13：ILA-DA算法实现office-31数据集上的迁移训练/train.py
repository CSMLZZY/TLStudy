...
if __name__ == '__main__':
    ...
    parser.add_argument('--source', type=str, nargs='?', default='a', help="source dataset")
    parser.add_argument('--target', type=str, nargs='?', default='w', help="target dataset")
    ...
    file_path = {
        "a": r"E:\\ILA-DA-main\\data\\amazon_31.txt",
        "d": r"E:\\ILA-DA-main\\data\\dslr_31.txt",
        "w": r"E:\\ILA-DA-main\\data\\webcam_31.txt",
    }

    dataset_source = file_path[args.source]
    dataset_target = dataset_test = file_path[args.target]

    batch_size = {"train": args.n_samples * num_categories, "val": args.n_samples * num_categories,
    ...
    my_discriminator = my_discriminator.to('cpu')
    ...
    msc_config = {
        ...
        'm': args.n_samples,
        'mu': args.mu,
        'n_per_domain': args.n_samples * num_categories,
    }
    msc_module = cf.MSCLoss(msc_config)
    msc_module = msc_module.to('cpu')
    ...
    for pt in range(1, args.pre_train + 1):
        ...
        inputs_source, labels_source = inputs_source.to('cpu'), labels_source.to('cpu')

        all_features, logits = my_net(inputs_source)
        classifier_loss = criterion["classifier"](logits, labels_source)
        writer.add_scalar("pre_train_classifier_loss", classifier_loss, pt)
    ...
    for iter_num in range(1, args.max_iteration + 1):
        ...
        bcel = nn.BCELoss()
        transfer_loss = bcel(domain_predicted, domain_labels)
        ...
        writer.add_scalars('train', {'classifier_loss': classifier_loss.item(), 'transfer_loss': transfer_loss.item(),
