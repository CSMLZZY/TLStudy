...
class MSCLoss(nn.Module):
    def __init__(self, config_data):
        ...
        self.n_per_domain = config_data['n_per_domain']
    ...
    def calc_loss(self, filtered_sim_matrix, src_labels, filtered_tgt_labels):
        ...
        if torch.cuda.is_available():
            same, diff, mask_sim = same.to('cpu'), diff.to('cpu'), mask_sim.to('cpu')

        final_mask = (same.bool() & diff.bool())

        if torch.cuda.is_available():
            filtered_sim_matrix, final_mask = filtered_sim_matrix.to('cpu'), final_mask.to('cpu')
