...
def train(config):
    ...
    best_acc = 0.0
    since = time.time()
    print("迭代总数：{:d}，每隔{:d}次迭代输出一次信息。".format(config['max_iter'], config['val_iter']))
    ...
    # for num_iter in tqdm(range(config['max_iter'])):
        if num_iter % config['val_iter'] == 0 and num_iter != 0:
            ...
            log_str = 'iter: {:d}, all_acc: {:.4f},\ttime: {:.4f}'.format(num_iter, temp_acc, time.time() - since)
            print(log_str)
        ...
        if num_iter % config['val_iter'] == 0:
            ...
            print('classifier_loss: {:.4f}\tmmd: {:.4f}\tmmd_seg: {:.4f}\tentropy: {:.4f}'.format(classifier_loss.item(),
                                                                                      transfer_loss.item(),
                                                                                      config['loss'][
                                                                                          'constant_off'] * sum_reg_loss,
                                                                                      entropy_loss.item() *
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_iter', type=int, default=15000, help='max iterations')
