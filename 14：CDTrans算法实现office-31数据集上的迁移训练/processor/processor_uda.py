...
def update_feat(cfg, epoch, model, train_loader1, train_loader2, device, feat_memory1, feat_memory2, label_memory1,
                label_memory2):
    model.eval()
    for n_iter, (img, vid, _, _, idx) in enumerate(tqdm(train_loader1)):
        with torch.no_grad():
            ...
            label_memory1[idx[0]:idx[-1] + 1] = vid
    ...
    for n_iter, (img, vid, _, _, idx) in enumerate(tqdm(train_loader2)):
        with torch.no_grad():
            ...
            label_memory2[idx[0]:idx[-1] + 1] = vid
...
def generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx, target_pseudo_label,
                         label_memory1, img_num1, img_num2, with_pseudo_label_filter=True):
    ...
    for idx, data in enumerate(tqdm(target_dataset)):
        ...
        if (with_pseudo_label_filter and mask) or not with_pseudo_label_filter:
            ...
            train_set.append(
                ((s_img_path, t_img_path), (label, target_pseudo_label[t_idx].item()), camid, trackid, (curidx, t_idx)))
    logger.info('target match accuracy')
    count_target_usage(logger, torch.tensor(new_targetidx), label_memory1, label_memory2, img_num1, img_num2,
    ...
    for idx, data in enumerate(tqdm(source_dataset)):
        ...
        if (with_pseudo_label_filter and mask) or not with_pseudo_label_filter:
            ...
            train_set.append(((s_img_path, t_img_path), (label, target_pseudo_label[curidx].item()), camid, trackid,
                              (s_idx, curidx.item())))

    logger.info('source match accuracy')
    count_target_usage(logger, torch.tensor(new_source_knnidx), label_memory1, label_memory2, img_num1, img_num2,
...
def do_train_uda(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch - 1) % update_epoch == 0:
            feat_memory1, feat_memory2, label_memory1, label_memory2 = update_feat(cfg, epoch, model, train_loader1,
                                                                                   train_loader2, device, feat_memory1,
                                                                                   feat_memory2, label_memory1,
                                                                                   label_memory2)
            dynamic_top = 1
            print('source and target topk==', dynamic_top)
            target_label, knnidx, knnidx_topk, target_knnidx = compute_knn_idx(logger, model, train_loader1,
                                                                               train_loader2, feat_memory1,
                                                                               feat_memory2, label_memory1,
                                                                               label_memory2, img_num1, img_num2,
                                                                               topk=dynamic_top, reliable_threshold=0.0)
            del train_loader

            train_loader = generate_new_dataset(cfg, logger, label_memory2, s_dataset, t_dataset, knnidx, target_knnidx,
                                                target_label, label_memory1, img_num1, img_num2,
        ...
        for n_iter, (imgs, vid, target_cam, target_view, file_name, idx) in enumerate(train_loader):
            ...
            with amp.autocast(enabled=True):
                ...
                (self_score1, self_feat1, _), (score2, feat2, _), (score_fusion, _, _), aux_data = model(img, t_img,
                                                                                                         target,
                                                                                                         cam_label=target_cam,
                                                                                                         view_label=target_view)  # output: source , target , source_target_fusion
            ...
            writer.add_scalar("train_Acc_avg", acc_meter.avg, nm)
            writer.add_scalar("train_Acc", acc, nm)

            torch.cuda.synchronize()
            if (n_iter + 1) % log_period == 0:
                writer.add_scalar("Train_Acc", acc_meter.avg, (epoch - 1) * (len(train_loader)) + n_iter)
                # writer.add_scalar("train_epoch{}_Acc".format(epoch), acc_meter.avg, n_iter + 1)
                logger.info(
                    "Epoch[{}] Iteration[{}/{}] Loss1: {:.3f}, Loss2: {:.3f}, Loss3: {:.3f},  Acc: {:.3f}, Acc2: {:.3f}, Acc2_pse: {:.3f}, Base Lr: {:.2e}"
                        .format(epoch, (n_iter + 1), len(train_loader),
                                loss1_meter.avg, loss2_meter.avg, loss3_meter.avg, acc_meter.avg, acc_2_meter.avg,
                                acc_2_pse_meter.avg, scheduler._get_lr(epoch)[0]))
            ...
            if cfg.MODEL.DIST_TRAIN:
                ...
                for n_iter, (img, vid, camid, camids, target_view, idex) in enumerate(val_loader):
                    with torch.no_grad():
                        ...
                        camids = camids.to(device)
                        target_view = target_view.to(device)
                        output_probs = model(img, img, cam_label=camids, view_label=target_view, return_logits=True,
                ...
                logger.info(
                    "Accuracy: {:.1%}, best Accuracy: {:.1%}, min Mean_entropy: {:.1}".format(accuracy, best_model_mAP,
                                                                                              min_mean_ent))
