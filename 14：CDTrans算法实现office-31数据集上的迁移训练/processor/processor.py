...
def do_train_pretrain(cfg,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for n_iter, (img, vid, target_cam, target_view, _) in enumerate(train_loader):
            ...
            writer.add_scalar("pre_train_Acc_avg", acc_meter.avg, nm)
            # writer.add_scalar("pre_train_Acc", acc, nm)

            torch.cuda.synchronize()
            if (n_iter + 1) % log_period == 0:
                writer.add_scalar("pre_train_Acc", acc_meter.avg, (epoch - 1) * (len(train_loader)) + n_iter)
                # writer.add_scalar("pre_train_epoch{}_Acc".format(epoch), acc_meter.avg, n_iter + 1)
        ...
        if epoch % eval_period == 0:
            if cfg.MODEL.DIST_TRAIN:
                ...
                writer.add_scalar("Pre_Train_Val_Acc", accuracy, epoch)
