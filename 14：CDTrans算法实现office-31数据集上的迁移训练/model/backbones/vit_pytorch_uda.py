...
class Block_3_branches(nn.Module):
    ...
    def forward(self, x, x2, x1_x2_fusion, use_cross=False, use_attn=True, domain_norm=False,
                inference_target_only=False):
        if inference_target_only:
            ...
            xa_attn, xa_attn2, xa_attn3, cross_attn = self.attn(self.norm1(x), self.norm1(x2),
                                                                inference_target_only=inference_target_only)
...
class ConvStemEmbed_stride(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride_size=20, in_chans=3, conv_channel=[24, 48, 96, 192],
                 embed_dim=768):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, x2, camera_id, view_id, domain_norm=False, cls_embed_specific=False,
                         inference_target_only=False):
        B = x.shape[0]
        ...
        if self.local_feature:
            ...
            if self.block_pattern == '3_branches':
                ...
                for i, blk in enumerate(self.blocks):
                    x, x2, x1_x2_fusion, cross_attn = blk(x, x2, x1_x2_fusion, use_cross=self.use_cross,
                                                          use_attn=self.use_attn, domain_norm=domain_norm,
                                                          inference_target_only=inference_target_only)
    ...
    def forward(self, x, x2, cam_label=None, view_label=None, domain_norm=False, cls_embed_specific=False,
                inference_target_only=False):
...
def uda_vit_small_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, drop_rate=0.0,
                                        attn_drop_rate=0.0, local_feature=False, aie_xishu=1.5, **kwargs):
    model = TransReID(
        img_size=img_size, patch_size=16, stride_size=stride_size, embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True,
        drop_path_rate=drop_path_rate, drop_rate=drop_rate, attn_drop_rate=attn_drop_rate, aie_xishu=aie_xishu,
...
def uda_vit_base_patch16_224_TransReID(img_size=(256, 128), stride_size=16, drop_path_rate=0.1, local_feature=False,
                                       aie_xishu=1.5, use_cross=False, use_attn=True, **kwargs):
    model = TransReID(
        ...
        norm_layer=partial(nn.LayerNorm, eps=1e-6), aie_xishu=aie_xishu, local_feature=local_feature,
        use_cross=use_cross, use_attn=use_attn, **kwargs)
