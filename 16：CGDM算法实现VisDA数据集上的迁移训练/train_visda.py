...
parser.add_argument('--test-batch-size', type=int, default=3, metavar='N',
                    help='input batch size for testing (default: 1000)')
parser.add_argument('--epochs', type=int, default=1, metavar='N', help='number of epochs to train (default: 20)')
    ...
    for ep in range(num_epoch):
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            for i in range(num_k):
                ...
                # print(pseudo_label_t)
                if ep > start:
                    gmn_loss = gradient_discrepancy_loss_margin(args, output_s1, output_s2, label_s, output_t1,
                                                                output_t2, pseudo_label_t, G, F1, F2)
                else:
                    gmn_loss = 0
            ...
            if batch_idx % 5 == 0:
                ...
                writer.add_scalars("train", {'cls_loss_F1': loss1.item(),
                                             'cls_loss_F2': loss2.item(),
                                             'dis_loss': loss_dis.item(),
                                             'entropy': entropy_loss.item()}, batch_idx)
