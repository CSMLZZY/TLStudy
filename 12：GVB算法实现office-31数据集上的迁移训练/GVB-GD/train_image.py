...
def train(config):
    ...
    for i in range(config["num_iterations"]):
        ...
        if i % config["test_interval"] == config["test_interval"] - 1:
            ...
            writer.add_scalar("test precision", temp_acc, i + 1)
        ...
        # loss calculation
        transfer_loss, mean_entropy, gvbg, gvbd = loss.GVB([softmax_out, focals], ad_net, network.calc_coeff(i),
                                                           GVBD=config['GVBD'])
        classifier_loss = nn.CrossEntropyLoss()(outputs_source, labels_source)
        total_loss = loss_params["trade_off"] * transfer_loss + classifier_loss + config["GVBG"] * gvbg + abs(
            config['GVBD']) * gvbd
        ...
        if i % config["print_num"] == 0:
            ...
            writer.add_scalar("train total loss", total_loss, i + 1)
            config["out_file"].write(log_str + "\n")
            config["out_file"].flush()
            # print(log_str)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--GVBG', type=float, default=0,
                        help="lambda: parameter for GVBG (if lambda==0 then GVBG is not utilized)")
    parser.add_argument('--GVBD', type=float, default=0,
                        help="mu: parameter for GVBD (if mu==0 then GVBD is not utilized)")
    ...
    config["out_file"] = open(osp.join("office", "log.txt"), "w")
    if not osp.exists(config["output_path"]):
